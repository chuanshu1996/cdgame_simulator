    ...
    <div class="team">
        ...
        <div class="team-section">
            <div class="team-title-row">
                ...
                <a-input
                    v-model="batchInputs[0]"
                    class="batch-input"
                    placeholder="粘贴选手名，逗号/顿号分隔，按位置自动填充"
                    @pressEnter="batchFill(0)"
                />
                <a-button type="default" class="batch-btn" @click="batchFill(0)">
                    批量填充
                </a-button>
            </div>
                            ...
                            <td class="col-select">
                                ...
                                >
                                    <a-select-option 
                                        ...
                                        :key="hero.index"
                                        :value="hero.index"
                                        :disabled="isHeroDisabled(0, hero.index)"
                                    >
                                        <div class="hero-option">
                                            <span class="hero-rank-tag" :class="'rank-' + hero.rank.toLowerCase()">{{ hero.rank }}</span>
                                            <span class="hero-name">{{ hero.name }}</span>
                                            <span v-if="getHeroExp(0, hero.index) >= 6" class="exp-tag maxed">满</span>
                                            <span v-else-if="getHeroExp(0, hero.index) >= 3" class="exp-tag qualified">{{ getHeroExp(0, hero.index) }}</span>
                                            <span v-else class="exp-tag unqualified">{{ getHeroExp(0, hero.index) }}</span>
                                        </div>
                                    </a-select-option>
                                </a-select>
                            </td>
        ...
        <div class="team-section">
            <div class="team-title-row">
                ...
                <a-input
                    v-model="batchInputs[1]"
                    class="batch-input"
                    placeholder="粘贴选手名，逗号/顿号分隔，按位置自动填充"
                    @pressEnter="batchFill(1)"
                />
                <a-button type="default" class="batch-btn" @click="batchFill(1)">
                    批量填充
                </a-button>
            </div>
            <div class="table-container">
                            ...
                            <td class="col-select">
                                ...
                                >
                                    <a-select-option 
                                        ...
                                        :key="hero.index"
                                        :value="hero.index"
                                        :disabled="isHeroDisabled(1, hero.index)"
                                    >
                                        <div class="hero-option">
                                            <span class="hero-rank-tag" :class="'rank-' + hero.rank.toLowerCase()">{{ hero.rank }}</span>
                                            <span class="hero-name">{{ hero.name }}</span>
                                            <span v-if="getHeroExp(1, hero.index) >= 6" class="exp-tag maxed">满</span>
                                            <span v-else-if="getHeroExp(1, hero.index) >= 3" class="exp-tag qualified">{{ getHeroExp(1, hero.index) }}</span>
                                            <span v-else class="exp-tag unqualified">{{ getHeroExp(1, hero.index) }}</span>
                                        </div>
                                    </a-select-option>
                                </a-select>
                            </td>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 智能匹配选手：支持姓名、昵称、缩略子序列匹配
            matchHero(query) {
                const q = this.normalize(query);
                if (!q) return null;
                let fallback = null; // 子序列匹配的结果（作为兜底）
                for (const hero of this.flatHeroList) {
                    ...
                    // 2. 子序列缩略匹配（长度>=2 才启用，避免单字误命中）
                    if (q.length >= 2) {
                        if (this.isSubsequence(q, nameN)) {
                            if (!fallback) fallback = hero.index;
            ...
            // 批量解析并填充到队伍（从教练 idx0 到 应援 idx7）
            batchFill(teamId) {
                const raw = this.batchInputs[teamId] || '';
                const names = this.splitNames(raw);
                if (names.length === 0) {
                    this.$message.warning('请输入选手名称');
                    return;
                }
                const team = teamId === 0 ? this.team0 : this.team1;
                const maxLen = team.length;
                let filled = 0;
                const failed = [];
                for (let i = 0; i < names.length && i < maxLen; i++) {
                    const no = this.matchHero(names[i]);
                    if (no === null || no === undefined) {
                        failed.push(names[i]);
                        continue;
                    }
                    this.handleSelectChange(teamId, i, no);
                    filled++;
                }
                if (failed.length > 0) {
                    this.$message.warning(
                        `已填充 ${filled} 个，以下未匹配：${failed.join('、')}`
                    );
                } else {
                    this.$message.success(`已成功填充 ${filled} 名选手`);
                }
            },
