<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
    <div class="debug" @click="closePanel">
        ...
        <div class="runway-field"> 
            <div v-for="item in data.runway" :key="item.entityId" 
                 class="runway-item" 
                 :class="{
                     team0: item.teamId === 0, 
                     team1: item.teamId === 1, 
                     judgeFlag: item.isJudgeFlag,
                     frozen: item.frozen, 
                     current: item.entityId === data.currentId, 
                     dead: item.dead
                 }" 
                 :style="{ left: Math.floor((item.distance / 100)) + '%' }"
                 @click="!item.isJudgeFlag && navigateToHeroList(item.no)">
                <img v-if="!item.isJudgeFlag" :src="getAvatarPath(item.no)" class="runway-avatar" />
                <span v-else class="judge-flag-count">{{ item.judgeFlagCount }}</span>
            </div>
        </div>
        <div class="team-field" v-for="teamId in 2" :key="teamId">
            <div class="energy-bar" :class="['team' + (teamId - 1)]">
                ...
                <div class="energy-orbs">
                    <span 
                        ...
                        class="energy-orb"
                        :class="[
                            n <= data.energy[teamId - 1].num ? 'filled' : '',
                            'team' + (teamId - 1),
                            (n === data.energy[teamId - 1].num + 1 && data.energy[teamId - 1].progress > 0) ? 'progressing' : ''
                        ]"
                    ></span>
                </div>
                <div class="energy-progress" v-if="data.energy[teamId - 1].progress > 0">
                    <span class="progress-label">回复中</span>
                    <span class="progress-value">{{ Math.floor(data.energy[teamId - 1].progress * 100) }}%</span>
                </div>
            </div>
            ...
            <div v-if="currentHero" class="card-detail">
                <div class="card-header">
                    <div class="card-avatar-section">
                        <img :src="getAvatarPath(currentHero.index)" class="card-avatar" />
                    </div>
...
<script>
    ...
    import {Battle, BattleProperties, Buff, EffectTypes, Reasons, SoulData, HeroBuilders, HeroData, JudgeFlagManager} from '../../core'
    ...
    function dump(battle) {
        ...
        battle.runway.distanceTable.forEach((distance, entityId) => {
            ...
            const isJudgeFlag = entity.name === '裁判旗';
            let judgeFlagCount = 0;
            if (isJudgeFlag) {
                const judgeFlag = JudgeFlagManager.getInstance().getJudgeFlag(battle);
                judgeFlagCount = judgeFlag.getStatus().judgeKingActionCount + 1;
            }
            dump.runway.push({
                entityId,
                distance,
        ...
        mounted() {
            ...
            for (let teamId = 0; teamId < 2; teamId++) {
                ...
                for (let posIndex = 0; posIndex < field.length; posIndex++) {
                    ...
                    if (entityId) {
                        // posIndex是战斗中的位置(0-5)，对应队伍设置中的索引也是posIndex(0-5)
                        const originalIndex = posIndex;
        ...
        methods: {
            ...
            runToEnd() {
                ...
                if (this.battle.isEnd) {
                    const winnerText = this.battle.winner === 0 ? '红队' : this.battle.winner === 1 ? '蓝队' : '平局';
                    message.info('战斗结束！' + (this.battle.winner >= 0 ? winnerText + '获胜' : '双方平局'));
            ...
            step() {
                ...
                do {
                    ...
                    !this.battle.isEnd && 
                    this.battle.currentTask.type !== 'WaitInput' && 
                    (this.depth === 0 || this.battle.currentTask.depth > this.depth)
                );
            ...
            useSkill(no, targetId) {
                ...
                if (!no) {
                    ...
                    const energy = this.battle.getEnergy(currentEntity.teamId);
                    const skills = this.data.skills;
                    
                    // 调用实体的AI方法获取技能选择
                    const selection = currentEntity.ai(this.battle, this.battle.currentTask.parent.data, energy, skills);
            ...
            updatePanel() {
                ...
                if (this.panelType === 'entity') {
                    ...
                    if (entity) {
                        ...
                        if (posIndex !== -1) {
                            const soulSelections = this.$store.state.soulSelections;
                            const soulIds = soulSelections && soulSelections[teamId] && soulSelections[teamId][posIndex];
            ...
            energyNum2Text(energy) {
                if (!energy) return '';

                return times(energy.num,() => '■').concat(times(8 - energy.num,() => '□')).join(' ')
            }
