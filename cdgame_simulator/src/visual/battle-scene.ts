...
import VisibleEnergy from "@/visual/visiable-energy";
...
export default class BattleScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.battle.entities.forEach((e) => {
            const avatarPath = getAvatarPathByNo(e.no).replace(/^\//, '');
            this.load.image(`avatar_${e.no}`, avatarPath);
        });
        console.log('preload');
    ...
    create() {
        ...
        for (let teamId = 0; teamId <= 1; teamId++) {
            ...
            const energy = this.energy[teamId] = new VisibleEnergy(this, dumpData.energy[teamId]);
            energy.x = this.game.canvas.width - 280;
            energy.y = 20 + teamId * 440;
            this.children.add(energy);
    ...
    update(_: number, __: number): void {
        ...
        for (let teamId = 0; teamId <= 1; teamId++) {
            ...
            this.energy[teamId].update(dumpData.energy[teamId], dumpData);
        }
    ...
    syncData() {
        ...
        //
        //
        // this.energy1 = this.battle.energys[0].num;
        // this.energy2 = this.battle.energys[1].num;
        // this.progress1 = this.battle.energys[0].progress;
        // this.progress2 = this.battle.energys[1].progress;
