...
export default function battleProcessor(battle: Battle, _: any, step: number): number {
    switch (step) {
        ...
        case 4: {
            // 处理伪回合（文档：伪回合优先于额外/普通回合）
            if (battle.fakeTurns.length) {
                battle.fakeTurns.forEach(ft => battle.addProcessor(fakeTurnProcessor, ft, '伪回合处理'));
                battle.fakeTurns = [];
            }
            // 处理额外回合（文档：回合结束阶段步骤11结算；行动条直接跳至100%）
            if (battle.extraTurns.length) {
                battle.extraTurns.forEach(et => {
                    const target = battle.getEntity(et.currentId);
                    if (!target.dead) {
                        // 核心特征：行动条无论在何处直接跳至100%
                        battle.runway.set(et.currentId, 10000);
                        const extraTurnProcessor = require('./extra-turn').default;
                        battle.addProcessor(extraTurnProcessor, et, '额外回合处理');
                    }
                });
                battle.extraTurns = [];
            }
