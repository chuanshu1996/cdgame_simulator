/**
 * 回合处理器
 * 负责处理游戏中的回合逻辑，包括回合开始、buff处理、能量条推进、技能选择和回合结束等
 */
...
export default function turnProcessor(battle: Battle, data: TurnProcessing, step: number): number {
    ...
    switch (step) {
        ...
        case 1: {
            ...
            // 输出回合开始日志（按裁判旗回合分组）
            battle.log(`[裁判旗回合#${battle.judgeRound}] 行动${battle.actionSeq} 队伍${currentEntity.teamId}·${currentEntity.name}`);
            // 添加回合开始日志
            battle.addEventLog('turn', `裁判旗回合#${battle.judgeRound} 行动${battle.actionSeq} 队伍${currentEntity.teamId}·${currentEntity.name}的回合开始`, {
        ...
        case 3: {
            // 推进当前队伍的能量条
            battle.actionUpdateEnergyProgress(0, currentEntity.teamId, 1, Reasons.RULE);
            return 4; // 进入回合内处理步骤
        }
        // 回合内
        case 4: {
            if (!data.cannotAction) {
                if (data.onlyAttack) {
                    // 被嘲讽，只能攻击嘲讽者
                    battle.actionUseSkill(1, currentEntity.entityId, data.onlyAttack, 0); //TODO: 是否需要确认能量
                } else if (data.confusion) {
                    // 混乱状态，随机攻击敌人
                    const target = battle.getRandomEnemy(currentEntity.entityId);
                    if (target) {
                        battle.actionUseSkill(1, currentEntity.entityId, target.entityId, 0); //TODO: 是否需要确认能量
                    }
                } else {
                    // 正常状态，选择技能
                    ...
                    const skills: SelectableSkill[] = currentEntity.skills
                        .filter(s => {
                            ...
                            if (cost > 0) {
                                if (!energy || energy.num < cost) return false; // 能量不足
                            }
                            return true;
                        })
                        .map(s => {
        ...
        case 5: {
            ...
            // 获取技能选择（手动输入或AI选择）
            const selection = (data.waitInput && data.waitInput.selection) || currentEntity.ai(battle, data, energy || null, data.skills);
