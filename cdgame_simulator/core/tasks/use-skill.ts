...
export default function useSkillProcessor(battle: Battle, data: UseSkillProcessing, step: number) {
    ...
    switch (step) {
        case 1: {
            ...
            // 消耗能量前触发事件，允许handler将cost设为0（不消耗能量）
            if (data.cost > 0) {
                battle.addEventProcessor(EventCodes.BEFORE_SKILL_COST, source.entityId, data);
            }
            return 2; // 进入消耗能量步骤
        }
        case 2: {
            // 消耗能量
            if (data.cost > 0) {
                battle.actionUpdateEnergy(source.entityId, source.teamId, -data.cost, Reasons.COST);

                // 记录能量消耗数据
                const currentEnergyUsed = Number(source.battleData.get('energyUsed')) || 0;
                source.battleData.set('energyUsed', String(currentEnergyUsed + data.cost));
            }
            return 3; // 进入执行技能效果步骤
        }
        case 3: {
