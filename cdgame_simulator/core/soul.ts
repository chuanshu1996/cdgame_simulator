...
export enum SoulEffectType {
    ...
    KILL_ADD_ENERGY = "kill_add_energy", // 击杀增加能量
}
...
export const SoulData: Soul[] = [
        ...
        },
    {
        ...
        name: "阴摩罗",
        description: "增加15%攻击力，击杀目标时增加3点能量",
        effects: [
            {
                type: SoulEffectType.PROPERTY_BUFF,
                propertyBuff: {
            ...
            {
                type: SoulEffectType.KILL_ADD_ENERGY,
                killEnergyAdd: 3,
                description: "击杀目标时增加3点能量"
            }
        ]
    }
];
...
export class SoulManager {
     ...
     * @returns 是否触发了增加能量
     */
    static checkAndTriggerKillAddEnergy(killer: Entity, battle: any): boolean {
        const souls = this.getEntitySouls(killer);
        if (souls.length === 0) return false;

        let totalEnergyAdd = 0;
        const triggeredSoulNames: string[] = [];

        souls.forEach(soul => {
            soul.effects.forEach(effect => {
                if (effect.type === SoulEffectType.KILL_ADD_ENERGY && effect.killEnergyAdd) {
                    totalEnergyAdd += effect.killEnergyAdd;
                    triggeredSoulNames.push(soul.name);
                }
            });
        });
        ...
        if (totalEnergyAdd > 0) {
            ...
            const energy = battle.getEnergy(teamId);
            energy.num = Math.min(energy.num + totalEnergyAdd, 8); // 能量上限为8

            // 添加日志
            const soulNames = triggeredSoulNames.join('、');
            battle.addEventLog('info', `【${killer.name}】的${soulNames}触发，获得${totalEnergyAdd}点能量`, {
                entityId: killer.entityId,
                energyAdd: totalEnergyAdd,
                soulNames: triggeredSoulNames
            });

            battle.log(`【${killer.name}】的${soulNames}触发，获得${totalEnergyAdd}点能量`);
