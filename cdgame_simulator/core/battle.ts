...
import {
    ...
    removeBuffProcessor,
    updateEnergyProcessor,
    UpdateEnergyProgressProcessing,
    updateEnergyProcessProcessor,
    UpdateRunWayProcessing,
    UseSkillProcessing,
    useSkillProcessor,
    UpdateEnergyProcessing,
    FakeTurnProcessing,
    ExtraTurnProcessing,
...
export interface BattleLog {
    ...
    message: string;
    time: string;
    judgeRound: number;     // 第几个裁判旗回合
    actionSeq: number;      // 该裁判旗回合内行动序号
    data?: any;
}
...
export default class Battle {
    output: object[]; // 游戏记录
    seed: number; // 随机种子
    energys: Energy[]; // 能量信息数组，每个队伍一个
    runway: Runway; // 行动条位置管理
    currentId: number; // 当前回合实体ID
    fields: number[][]; // 场上位置，二维数组，每个队伍一个数组
    fieldSize: number; // 场地大小
    turn: number; // 当前回合数（第几次行动）
    judgeRound: number; // 裁判旗回合计数（每次裁判旗行动+1）
    actionSeq: number; // 当前裁判旗回合内行动序号
    ...
    extraTurns: ExtraTurnProcessing[]; // 额外回合处理列表（行动条跳100%后立刻行动）
     ...
     * @returns 伪回合索引
     */
    addFakeTurn(currentId: number, processor: Processor, data: any = {}, fakeType: 'A' | 'B' | 'C' | 'NORMAL' = 'NORMAL') {
        const ft = new FakeTurnProcessing(processor, data, currentId, fakeType);
    ...
    /**
     * 添加额外回合处理
     * 对应《回合结算过程3.0》"额外回合"：目标行动条直接跳至100%，并在回合结束阶段结算。
     * 由妖琴师〔余音〕、神乐〔疾风〕、镰鼬〔人多势众〕、鬼使黑〔索命〕、
     * 凤凰火〔凤凰业火〕、〈轮入道〉等技能调用。
     * @param currentId 当前实体ID（获得额外回合者）
     * @param opts 来源技能编号与触发者ID（用于日志/去重）
     * @returns 额外回合索引
     */
    addExtraTurn(currentId: number, opts: { sourceSkillNo?: number; sourceId?: number } = {}) {
        const existingIndex = this.extraTurns.findIndex(et => et.currentId === currentId);
        if (existingIndex !== -1) {
            // 同名覆盖（避免同一来源重复堆叠）
            this.extraTurns.splice(existingIndex, 1);
        }
        const et = new ExtraTurnProcessing(++this.turn, currentId, opts);
        return this.extraTurns.push(et);
    }
     ...
     * @returns 能量对象
     */
    getEnergy(teamId: number): Energy {
        if (!this.energys[teamId]) throw new Error('Cannot get energy by teamId = ' + teamId);

        return this.energys[teamId];
    }
     ...
     * @param reason 原因
     */
    actionUpdateEnergy(sourceId: number, teamId: number, num: number, reason: Reasons = Reasons.NOTHING) {
        this.addProcessor(updateEnergyProcessor, new UpdateEnergyProcessing(sourceId, teamId, num, reason), 'UpdateEnergy');
    }
     ...
     * @returns 任务ID
     */
    actionUpdateEnergyProgress(sourceId: number, teamId: number, num: number, reason: Reasons = Reasons.NOTHING) {
        return this.addProcessor(updateEnergyProcessProcessor, new UpdateEnergyProgressProcessing(sourceId, teamId, num, reason), 'ProcessEnergyProgress');
    }
    ...
    flushPendingDamageLogs() {
        this.pendingDamageLogs.forEach((log) => {
            const sourceName = this.resolveEntityName(log.sourceId, log.sourceName);
            const targetName = this.resolveEntityName(log.targetId, log.targetName);
            const critText = log.criticalCount > 0 ? `(含${log.criticalCount}次暴击)` : '';
            const hitText = log.hitCount > 1 ? `${log.hitCount}段` : '';
            const message = `${sourceName}使用[${log.skillName}]${hitText}对${targetName}造成了${Math.round(log.totalDamage)}点伤害${critText}`;
    ...
    /**
     * 解析实体显示名：优先使用已记录的 name，若为 <Unknown>/空则回退到实体表实时查询
     */
    private resolveEntityName(id: number, fallback: string): string {
        if (fallback && fallback !== '<Unknown>') return fallback;
        const entity = this.getEntity(id);
        if (entity && entity.name && entity.name !== '<Unknown>') return entity.name;
    ...
    flushPendingBuffLogs() {
        this.pendingBuffLogs.forEach((log) => {
            ...
            let effectText = '';
            if (log.effect) {
                const eff = log.effect;
                // FIXED/SET 为固定数值，直接使用；ADD_RATE 为百分比倍率，需乘100加%
                const isPercent = eff.effectType === EffectTypes.ADD_RATE;
                const displayValue = isPercent ? Math.round(eff.value * 100) : Math.round(eff.value);
                const unit = isPercent ? '%' : '';
                effectText = `效果:${eff.propertyName}${eff.value > 0 ? '+' : ''}${displayValue}${unit}`;
            }
            
            const targetName = this.resolveEntityName(log.targetId, log.targetName);
            const sourceName = this.resolveEntityName(log.sourceId, log.sourceName);
            const message = `${targetName}获得了[${log.buffName}]${stackText}${maxText}${effectText ? '，' + effectText : ''}`;
    ...
    /**
     * 构建裁判旗回合行动序列汇总
     * 按 judgeRound 分组，列出每个回合内各角色的行动顺序与次数
     * @returns 结构化汇总对象数组
     */
    buildRoundActionSummary(): Array<{
        judgeRound: number;
        actionCount: number;
        actions: Array<{ actionSeq: number; teamId: number; entityId: number; name: string }>;
    }> {
        const turnLogs = this.eventLogs.filter(l => l.type === 'turn' && l.data && typeof l.data.entityId === 'number');
        const map = new Map<number, Array<{ actionSeq: number; teamId: number; entityId: number; name: string }>>();
        for (const l of turnLogs) {
            const d = l.data;
            if (!map.has(l.judgeRound)) map.set(l.judgeRound, []);
            map.get(l.judgeRound)!.push({
                actionSeq: l.actionSeq,
                teamId: d.teamId,
                entityId: d.entityId,
                name: String(l.message).replace(/^裁判旗回合#\d+ 行动\d+ 队伍\d+·/, '').replace(/的回合开始$/, ''),
            });
        }
        const result: Array<any> = [];
        Array.from(map.keys()).sort((a, b) => a - b).forEach(round => {
            const actions = map.get(round)!.sort((a, b) => a.actionSeq - b.actionSeq);
            result.push({
                judgeRound: round,
