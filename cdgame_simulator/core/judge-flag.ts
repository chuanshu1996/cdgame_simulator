...
export class JudgeFlag {
    ...
    getStatus(): {
        ...
        const healMultiplier = this.getHealMultiplier();
    ...
    /**
     * 获取治疗倍率
     * 裁判旗回合 1~4 期间治疗量保持完整（倍率 1.00x，不受削弱）；
     * 从第 5 回合起按每层 0.20 逐步降低治疗效果强度，最低封底 0.20x。
     */
    getHealMultiplier(): number {
        // 总层数即裁判旗行动次数（回合数）；前 4 回合不削弱
        const totalLayers = this.normalLayers + this.ultimateLayers;
        if (totalLayers <= 4) return 1.0;
        const reducedLayers = totalLayers - 4; // 从第 5 回合起才开始累计削弱层数
        const totalHealReduction = Math.min(reducedLayers * 0.20, 0.8);
...
export function triggerJudgeKingAction(battle: Battle): void {
    ...
    // 记录裁判旗回合分组头
    battle.log(`========== 裁判旗回合 #${battle.judgeRound} 开始 ==========`);
    ...
    // 添加事件日志
    battle.addEventLog('info' as BattleLogType, `裁判旗回合 #${battle.judgeRound} 开始（裁决鬼王第${newStatus.judgeKingActionCount}次行动）`, {
        judgeRound: battle.judgeRound,
        judgeKingActionCount: newStatus.judgeKingActionCount,
