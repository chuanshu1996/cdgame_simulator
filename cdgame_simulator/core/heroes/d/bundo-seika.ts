...
export const bundo_seika_skill1: Skill = {
    ...
    text: '兴奋地收集各种卡牌，对单体目标造成攻击力125%的伤害（队伍中有津山睦月时提升至攻击力150%）。',
    use(battle: Battle, sourceId: number, selectedId: number) {
        const source = battle.getEntity(sourceId);
        if (!source) return;
        
        const hasAlly = hasTsuyamaMutsuki(battle, source.teamId);
        const rate = hasAlly ? 1.5 : 1.25;
...
function buildResistBuff(sourceId: number, targetId: number, count: number): Buff {
    return Buff.build(sourceId, targetId)
        .name('呼唤队长·抵抗', 1)
        .buffAP(BattleProperties.EFT_RES, EffectTypes.ADD_RATE, 0.1 * count)
        .end();
}
...
export const bundo_seika_skill2: Skill = {
    ...
    passive: true,
    cost: 0,
    text: '被动技能。当自身生命值低于50%时触发，获得【呼唤队长·攻击】buff，攻击力提升10%，持续至战斗结束；同时自身每持有1层【稳健】buff，额外获得10%效果抵抗（以【呼唤队长·抵抗】buff呈现）。',
    handlers: [
        {
...
export const bundo_seika_skill3: Skill = {
    ...
    cost: 3,
    target: SkillTarget.ENEMY,
    text: '牺牲自身1000点生命值，对单体目标造成攻击力280%的伤害；若造成的伤害超过5000点，则获得1层【稳健】buff。【稳健】：防御力提升10%，最多叠加5层，持续至战斗结束。',
    use(battle: Battle, sourceId: number, selectedId: number) {
        const source = battle.getEntity(sourceId);
        if (!source) return;
