...
export const mizumura_shiori_skill1: Skill = {
    ...
    text: '使用镜子折射光线，对单体目标造成攻击力50%的真实伤害。（己方存在【化妆镜】召唤物时，额外对另一位随机敌人造成相同伤害，该伤害来源视为化妆镜）',
...
export const mizumura_shiori_skill2: Skill = {
    ...
    passive: true,
    cost: 0,
    text: '被动技能。当自身受到一次伤害时，有10%基础概率（受效果命中加成）对伤害来源施加【着迷】debuff（己方存在【化妆镜】召唤物时概率提升至15%）。【着迷】：持续至战斗结束，每回合有50%概率无法进行攻击（仍可释放非攻击型技能）。',
    handlers: [
        {
...
export const mizumura_shiori_skill3: Skill = {
    ...
    text: '己方不存在召唤物时，召唤【化妆镜】，其生命值继承自身30%最大生命值。',
    use(battle: Battle, sourceId: number, selectedId: number) {
        const source = battle.getEntity(sourceId);
        if (!source) return;
 ...
 * @returns 技能选择对象
 */
export function mizumuraShioriAI(battle: Battle, turnData: TurnProcessing, energy: Energy | null, selections: SelectableSkill[]): SkillSelection | null {
    ...
    // 没有召唤物且能量足够时，优先使用技能3召唤化妆镜
    if (!hasSummon && currentEnergy >= 3) {
