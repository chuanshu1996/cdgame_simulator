...
export const ido_sumiko_skill1: Skill = {
    ...
    text: '优先攻击技能名含"立直"的目标，对单体目标造成攻击力100%的伤害；若目标技能名含"立直"，则改为造成攻击力200%的伤害，并使自身获得【兜牌】buff。【兜牌】：速度降低10点，防御力提升30%，最多叠加1层，持续1回合。',
...
export const ido_sumiko_skill3: Skill = {
    ...
    text: '对单体目标造成攻击力100%的真实伤害；若自身拥有【兜牌】buff，则改为造成攻击力200%的真实伤害，并返还1点能量。',
    use(battle: Battle, sourceId: number, selectedId: number) {
        const source = battle.getEntity(sourceId);
        if (!source) return;
        ...
        if (hasDoupai) {
            ...
            battle.log(`【${source.name}】使用【默默默听】，有兜牌buff，返还1点能量，造成200%真实伤害`);
        ...
        battle.addEventLog('skill', `【${source.name}】使用【默默默听】，造成${damageRate * 100}%真实伤害${hasDoupai ? '，返还1点能量' : ''}`, {
            sourceId: sourceId,
            targetId: selectedId,
            damageRate: damageRate,
            hasDoupai: hasDoupai,
 ...
 * 有兜牌buff时，优先使用技能3（默默默听）来消耗buff并返还能量
 * @param battle 战斗对象
 * @param turnData 回合数据
 * @param energy 能量对象
 ...
 * @returns 技能选择对象
 */
export function idoSumikoAI(battle: Battle, turnData: TurnProcessing, energy: Energy | null, selections: SelectableSkill[]): SkillSelection | null {
    const currentEntity = battle.getEntity(turnData.currentId);
    if (!currentEntity) return normalAI(battle, turnData, energy, selections);
    ...
    // 技能3消耗1能量，但会返还1能量，所以实际上不消耗能量
    if (hasDoupai && currentEnergy >= 1) {
    ...
    // 其他情况使用默认AI
    return normalAI(battle, turnData, energy, selections);
}