...
export const honnai_naruka_skill1: Skill = {
    ...
    text: '稳扎稳打，对敌方全体造成攻击力2%的真实伤害（队伍中有桧森誓子时提升至攻击力4%）。',
...
export const honnai_naruka_skill2: Skill = {
    ...
    passive: true,
    cost: 0,
    text: '被动技能。当自身受到一次攻击时，获得1层【祈愿防御】buff；当自身造成一次伤害时，获得1层【祈愿进攻】buff。当队友使用一次消耗能量的技能时，有（队伍中【祈愿】标签人数×10%）的概率不消耗能量（队伍包含队伍设置中的8个位置）。【祈愿防御】：每层使防御力提升25点，持续至战斗结束。【祈愿进攻】：每层使攻击力提升20点，持续至战斗结束。',
    handlers: [
        {
        ...
        {
            handle(battle: Battle, data: RealEventData) {
                // data.eventId 是使用技能的人，data.skillOwnerId 是本内成香
                ...
                // 统计队伍中（包括替补和应援8个位置）拥有【祈愿】标签的人数
                const teamEntities = battle.getTeamEntities(skillUser.teamId);
                let qiyuanCount = 0;
                for (const entity of teamEntities) {
                    if (entity.dead) continue;
                    const heroInfo = HeroData.find(d => Number(d.index) === entity.no);
                    if (heroInfo && heroInfo.label && heroInfo.label.split('，').includes('祈愿')) {
                        qiyuanCount++;
                    }
                }
                
                if (qiyuanCount <= 0) return -1;
...
export const honnai_naruka_skill3: Skill = {
    ...
    limited: true,
    target: SkillTarget.ENEMY,
    text: '【限定技】当自身生命值低于25%时可使用，整场战斗仅限一次。对敌方全体造成攻击力150%的真实伤害，并将造成的伤害量转化为护盾给予自身，持续2回合。',
    use(battle: Battle, sourceId: number, selectedId: number) {
        const source = battle.getEntity(sourceId);
        if (!source) return;
