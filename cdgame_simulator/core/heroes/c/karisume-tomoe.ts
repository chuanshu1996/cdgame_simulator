...
export const karisume_tomoe_skill1: Skill = {
    ...
    text: '对单体目标造成攻击力100%的间接伤害；队伍中每有1位巫女角色，本次伤害提升20%。',
...
export const karisume_tomoe_skill2: Skill = {
    ...
    passive: true,
    cost: 0,
    text: '被动技能。【先机】为所有巫女角色施加增益：队伍中每有1位巫女，速度提升10点、效果抵抗提升10点、护盾提升1000点，持续1回合。（替补位和应援位也可以触发）',
    handlers: [
        {
...
export const karisume_tomoe_skill3: Skill = {
    ...
    text: '为自身及大将驱散全部减益效果，并为其余所有队友各驱散1个随机减益效果。',
    use(battle: Battle, sourceId: number, selectedId: number) {
        const source = battle.getEntity(sourceId);
        if (!source) return;
